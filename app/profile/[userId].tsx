...
function resolveProfileImageUri(uri: string | null | undefined): string | null {
  ...
  return assetBase
    ? `${assetBase}${uri.startsWith("/") ? uri : `/${uri}`}`
    : uri;
}
...
export default function OtherUserProfileScreen() {
  ...
  const { viewedUser, stats, isLoading } = useAppSelector(
    (state) => state.profile,
  );
    ...
    apiService
      ...
      .then((res) => {
        ...
        if (res.success && res.data?.blocked_users) {
          setBlockedUserIds(
            new Set(
              res.data.blocked_users.map((b) => String(b.blocked_user_id)),
            ),
          );
        }
      })
  ...
  const instagramId =
    extractInstagramIdFromUrl(socialMedia.instagram) ||
    socialMedia.instagram?.replace(/^@/, "") ||
    "";
  const instagramUrl =
    getInstagramProfileUrl(socialMedia.instagram) ||
  ...
  const instagramGridImages = isBusinessProfile
    ? recentPlans
        .flatMap((p) => (p.media || []).map((m: any) => m.url))
        .filter(Boolean)
        .slice(0, 9)
    : [];
  ...
  const socialCardEntries: {
  ...
  ] : [
    {
      key: "instagram",
      icon: "logo-instagram",
      handle: instagramId || socialMedia.instagram || "Not added",
      url:
        instagramUrl ||
      ...
      ? [
          {
            ...
            icon: "document-text",
            label: "Google Drive",
            url: socialMedia.google_drive.startsWith("http")
              ? socialMedia.google_drive
              : `https://drive.google.com/${socialMedia.google_drive}`,
            color: "#4285F4",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            icon: "x",
            label: "X",
            url: `https://x.com/${String(socialMedia.x || socialMedia.twitter).replace(/^@/, "")}`,
            color: "#000000",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            icon: "logo-snapchat",
            label: "Snapchat",
            url: socialMedia.snapchat.startsWith("http")
              ? socialMedia.snapchat
              : `https://www.snapchat.com/add/${String(socialMedia.snapchat).replace(/^@/, "")}`,
            color: "#FFFC00",
          },
        ]
      : []),
  ...
  return (
    <View style={styles.container}>
      ...
      <View
        style={[styles.stickyBackOverlay, { paddingTop: insets.top + 12 }]}
        pointerEvents="box-none"
      >
        <TouchableOpacity
        ...
        >
          ...
          {isBusinessProfile ? (
            <>
                    ...
                    <React.Fragment key={entry.key}>
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.socialHandle,
                            !entry.url && styles.socialHandlePlaceholder,
                          ]}
                        >
                          {entry.label}
                        </Text>
              ...
              <View key="business-content" style={styles.businessContentCard}>
                ...
                <View style={styles.businessInstaSection}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      instagramUrl && setShowInstagramWebView(true)
                    }
                    activeOpacity={0.8}
                  >
                    <Ionicons name="logo-instagram" size={24} color="#E4405F" />
                    <Text style={styles.businessInstaHandle}>
                ...
                {instagramUrl && (
                  ...
                  >
                    <SafeAreaView
                      style={styles.instagramWebViewContainer}
                      edges={["top"]}
                    >
                      <View style={styles.instagramWebViewHeader}>
                        <TouchableOpacity
                          onPress={() => setShowInstagramWebView(false)}
                          style={styles.instagramWebViewCloseBtn}
                        >
                          <Ionicons name="close" size={28} color="#1C1C1E" />
                        </TouchableOpacity>
                        <Text
                          style={styles.instagramWebViewTitle}
                          numberOfLines={1}
                        >
                          @{instagramId || viewedUser?.name || "instagram"}
                        </Text>
                        <TouchableOpacity
                          ...
                          style={styles.instagramWebViewOpenBtn}
                        >
                          <Ionicons
                            name="open-outline"
                            size={22}
              ...
              <View style={styles.businessPreviousRunsCard}>
                <View style={styles.businessPreviousRunsSection}>
                  <View style={styles.businessPreviousRunsHeader}>
                    <Text style={styles.businessPreviousRunsTitle}>
                      Previous Runs
                    </Text>
                  ...
                  {loadingPlans ? (
                    ...
                    <ScrollView
                      ...
                      contentContainerStyle={
                        styles.businessPreviousRunsScrollContent
                      }
                      ...
                      {recentPlans.map((plan, planIndex) => {
                        ...
                        return (
                          <TouchableOpacity
                            key={planKey}
                            style={[
                              styles.businessPreviousRunCardNew,
                              { width: recentPlanCardWidth },
                            ]}
                            activeOpacity={0.95}
                            onPress={() =>
                              router.push({
                          ...
                          >
                            <View
                              style={styles.businessPreviousRunCardImageWrap}
                            >
                              {hasImage ? (
                                <Image
                                ...
                                <View
                                  style={[
                                    styles.businessPreviousRunCardImage,
                                    styles.businessPreviousRunCardPlaceholder,
                                  ]}
                                />
                              )}
                            </View>
                            <View
                              style={styles.businessPreviousRunCardWhiteStrip}
                            >
                              <Text
                                style={styles.businessPreviousRunCardTitleNew}
                                numberOfLines={2}
                              >
                              ...
                              <Text
                                style={
                                  styles.businessPreviousRunCardDescriptionNew
                                }
                                numberOfLines={2}
                              >
              ...
              {isOwnProfile && latestBusinessPlanLink && (
                <View style={styles.businessOwnerShareWrap}>
                  <TouchableOpacity
                    style={styles.businessOwnerShareButton}
                    onPress={handleShareLatestPlanLink}
                    activeOpacity={0.8}
                  >
                    <Ionicons
                      ...
                      color="#1C1C1E"
                    />
                    <Text style={styles.businessOwnerShareText}>
                      Share latest plan link
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.businessOwnerCopyButton}
                    onPress={handleCopyLatestPlanLink}
                    activeOpacity={0.8}
                  >
                    <Ionicons name="copy-outline" size={18} color="#1C1C1E" />
                    <Text style={styles.businessOwnerShareText}>
                      Copy latest plan link
                    </Text>
                  </TouchableOpacity>
                </View>
              )}
                ...
                <View style={styles.businessButtonsWrap}>
                  ...
                  >
                    <Ionicons
                      ...
                      color="#FF3B30"
                    />
                    <Text style={styles.businessReportButtonText}>
                      {blockingBusy
                        ? "Please wait…"
                        : isBlocked
                          ? "Unblock"
            ...
            <>
              ...
              <View
                key="social-card"
                style={[styles.sectionCard, styles.socialCardFigma]}
              >
              ...
              <View
                key="stats-card"
                style={[styles.statsCardFigma, { marginBottom: 8 }]}
              >
                <View style={styles.statsCardInner}>
                  <View style={styles.statHalf}>
                    <Text style={styles.statNumberFigma}>
                      {stats?.plans_count ?? 0}
                    </Text>
              ...
              {interests.length > 0 && (
                <View
                  key="interests-card"
                  style={[styles.interestsCardFigma, { marginBottom: 8 }]}
                >
                  ...
                  <View style={styles.interestsRowWrap}>
                    {interests.slice(0, 9).map((interest) => (
                      <View key={interest} style={styles.interestPillFigma}>
                        <Ionicons
                          name={(INTEREST_ICONS[interest] || "pricetag") as any}
                          size={20}
                          color="#3b3c3d"
                        />
              ...
              <View
                key="recent-plans-section"
                style={styles.recentPlansSectionFigma}
              >
                ...
                >
                  <Pressable
                    style={styles.recentPlansModalOverlay}
                    onPress={() => setRecentPlansModalVisible(false)}
                  >
                    <View
                      style={styles.recentPlansModalContent}
                      onStartShouldSetResponder={() => true}
                    >
                      <View style={styles.recentPlansModalHeader}>
                        <Text style={styles.recentPlansModalTitle}>
                          Recent Plans
                        </Text>
                        <TouchableOpacity
                          onPress={() => setRecentPlansModalVisible(false)}
                      ...
                      <ScrollView
                        ...
                        contentContainerStyle={
                          styles.recentPlansModalScrollContent
                        }
                    ...
                    {!isOwnProfile && (
                      <>
                        ...
                        >
                          <Ionicons
                            name={
                              isBlocked ? "lock-open-outline" : "ban-outline"
                            }
                            size={24}
                            color="#FF3B30"
                          />
                ...
                <View key="actions-report-only" style={{ gap: 10 }}>
                  ...
                  >
                    <Ionicons
                      ...
                      color="#FF3B30"
                    />
                    <Text style={styles.reportOnlyText}>
                      {blockingBusy
                        ? "Please wait…"
                        : isBlocked
                          ? "Unblock User"
